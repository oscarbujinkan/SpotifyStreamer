...
public class TopTracksResultAdapter extends RecyclerView.Adapter<TopTracksResultHolder>{
    ...
    @Override
    public TopTracksResultHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
    ...
    @Override
    public void onBindViewHolder(final TopTracksResultHolder topTracksResultHolder, final int i) {
        if(i< mTracks.tracks.size()) {
            ...
            if(track.album.images.size()>0) {
                Picasso.with(mContext).load(track.album.images.get(track.album.images.size()-1).url).into(topTracksResultHolder.topTrackImage, new Callback() {
                    @Override
